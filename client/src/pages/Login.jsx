...
function normalizeUser(user, form) {
  const fallback = buildFallbackProfile(form);
  const safe = user || fallback;

  return {
    id: safe.id || fallback.id,
    username: safe.username || safe.name || fallback.username,
    name: safe.name || safe.username || fallback.name,
    email: safe.email || fallback.email,
    role: safe.role || fallback.role,
    location: safe.location || safe.country || fallback.location,
    country: safe.country || safe.location || fallback.country,
    company: safe.company || safe.org || fallback.company,
    org: safe.org || safe.company || fallback.org,
    joinedAt: safe.joinedAt || fallback.joinedAt,
  };
}
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      try {
        ...
        userProfile = result?.data?.user || result?.user || null;
      } catch {
        result = loginLocalUser(form);
        userProfile = result?.user || result?.data?.user || null;
      }

      if (!userProfile) {
        const registeredUser = JSON.parse(
          localStorage.getItem('trafficSense_registered_user') || 'null'
        );

        if (
          registeredUser &&
          String(registeredUser.email).toLowerCase() === String(form.email).toLowerCase()
        ) {
          userProfile = registeredUser;
        }
      }

      const normalizedProfile = normalizeUser(userProfile, form);

      localStorage.setItem('trafficSense_auth', JSON.stringify(result));
      localStorage.setItem('trafficSense_user', JSON.stringify(normalizedProfile));
    ...
    <main className="app-soft-bg flex min-h-screen items-center justify-center p-6 text-slate-900">
      <form onSubmit={handleSubmit} className="glass-card w-full max-w-md rounded-3xl p-8">
        ...
        <label className="mt-5 block">
          ...
          <div className="mt-2 flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 shadow-sm backdrop-blur-xl focus-within:border-cyan-400 focus-within:ring-4 focus-within:ring-cyan-400/10">
            <input
              ...
              required
            />
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-slate-400 hover:text-cyan-600">
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-8 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-400 to-blue-600 px-6 py-4 font-extrabold text-white shadow-lg shadow-cyan-500/20 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
        >
          <LogIn size={18} />
