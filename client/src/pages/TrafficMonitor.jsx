...
export default function TrafficMonitor() {
  ...
  const onlineCount = cameras.filter(
    (camera) => camera.network_status === 'Online'
  ).length;
      ...
      <div className="space-y-4">
        <div className="panel p-4">
          ...
          <div className="mt-4 flex items-center justify-between text-sm text-slate-400">
            <span>
              {onlineCount}/{cameras.length} Kamera Online
            </span>
            <RefreshCcw size={16} />
          </div>
        </div>
        ...
        <div className="panel overflow-hidden p-4 md:p-5">
          <div className="relative overflow-hidden rounded-[28px]">
            ...
            <div className="absolute left-4 top-4 flex items-center gap-3">
              ...
              <span className="rounded-xl bg-black/55 px-3 py-2 text-sm">
                30 FPS
              </span>
            </div>

            <div className="absolute right-4 top-4 flex items-center gap-3">
              <button className="rounded-full bg-black/55 p-3 text-white">
                <Volume2 size={18} />
              </button>
              <button className="rounded-full bg-black/55 p-3 text-white">
                <Expand size={18} />
            ...
            {(activeCamera.boxes || []).map((box, index) => (
              <div
                key={index}
                className="absolute rounded-md border-2 border-cyan-300"
                style={{
                  left: `${box.x}%`,
                  top: `${box.y}%`,
                  width: `${box.w}%`,
            ...
            <div className="absolute bottom-5 right-5 text-right">
              ...
              <p className="mt-3 text-lg text-slate-100">
                {activeCamera.vehicles_detected} kendaraan terdeteksi
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-4">
            ...
            <Stat
              ...
              value={activeCamera.network_status}
              tone={
                activeCamera.network_status === 'Online'
                  ? 'text-emerald-400'
                  : 'text-rose-400'
              }
            />
